...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    ...
    fnt = ImageFont.truetype('code/miscc/FreeMono.ttf', 50)
...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
                        attn_maps, att_sze, vis_size=256, topK=5, save_mask=None,save_dir=None):
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            minV = one_map.min()
            maxV = one_map.max()
            one_map = (one_map - minV) / (maxV - minV + 1e-6)
            row_beforeNorm.append(one_map)
        ...
        for j in range(num_attn):
            ...
            if save_mask is not None and j == save_mask[i]:
                PIL_att.save(os.path.join(save_dir,'attention{}.png'.format(i)))
                mask_arr.append(PIL_att)
            ...
            merged.paste(PIL_im, (0, 0))
            merged.paste(PIL_att, (0, 0), mask)
            # merged.save('/home/pengjiazhen/DM-GAN+CL/output/{}.png'.format(j))
            merged = np.array(merged)[:, :, :3]
        ...
        for j in range(num_attn):
            img_set[i]=np.pad(img_set[i],((0,0),(0,max_width-img_set[i].shape[1]),(0,0)))
    ...
    if bUpdate:
        ...
        img_set = img_set.astype(np.uint8)
        if save_mask!=None:
            return img_set, mask_arr, sentences
        return img_set, sentences
    else:
        return None
...
def get_code(sentence):
    hex_num=hashlib.md5(sentence.encode(encoding='utf-8')).hexdigest()
    num=int('0x'+hex_num,16)
    a=1<<32
    code=np.zeros(512,dtype='float32')
    for i in range(4):
        seed=num%a
        code[i*128:(i+1)*128]=np.random.RandomState(seed).randn(1, 128)
        num=num>>32
    return code

def load_dataset_dict_info():
